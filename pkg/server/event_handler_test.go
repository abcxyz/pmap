...
isOK: true`),
		...
		},
		{
			...
			wantPmapEvent: &v1alpha1.PmapEvent{},
			wantAttr: map[string]string{
				AttrKeyProcessErr: fmt.Sprintf("failed to get GCS object: pmap process err: failed to unmarshal object yaml: failed to unmarshal yaml: %s",
...
func (m *testMessenger) Send(_ context.Context, data []byte, attr map[string]string) error {
	...
	// len(data) can be zero when generatePmapEventBytes() fails.
	// if len(data) is zero, there is no point to Unmarshal data,
	// the unmarshal here is to help test generatePmapEventBytes() generated
	// the right eventBytes.
	if len(data) != 0 {
